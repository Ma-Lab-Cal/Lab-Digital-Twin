...
class FlamingoWS:
    def __init__(self, addr=("localhost", 8000), n_connections=8):
        ...
        self.db = sqlite3.connect("C:\\Users\\ma-la\\Documents\\Digital-Twin-Inventory-System\\networktable.db", check_same_thread=False)
        ...
        if is_HTTP_request:
            ...
            if request.headers.get("Connection") == "Upgrade" and request.headers.get("Upgrade") == "websocket":
                ...
                if data.get("method") == "GET":
                    if data.get("params"):
                        entries = self.db.cursor().execute("""SELECT * FROM NetworkTable WHERE key LIKE "{key}";""".format(key=data["params"].get("key"))).fetchall()
                        print(entries)
                        return_data = json.dumps(entries).encode()
                        stream.transmit(return_data+b"\n")

                if data.get("method") == "SET":
                    if data.get("params"):
                        key = data["params"].get("key")
                        value = json.dumps(data["params"].get("value")).replace("\"", "\"\"")
                        exe_str = """INSERT INTO NetworkTable (key, value) VALUES ("{key}", "{value}") ON CONFLICT(key) DO UPDATE SET value="{value}";""".format(key=key, value=value)
                        print(exe_str)
                        self.db.cursor().execute(exe_str)
                        return_data = json.dumps({"method": "RET", "params": {"message": "success"}}).encode()
                        stream.transmit(return_data+b"\n")
                        self.db.commit()

                # return_data = json.dumps({"method": "RET", "params": {"message": "nope."}}).encode()
